        ...
        <main class="flex-1 p-6 ml-64">
            <div>
                ...
                <div class="flex items-start justify-between mb-6">
                    ...
                    <button type="button" onclick="exportApplications()" class="px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-400 text-white rounded-lg hover:from-blue-600 hover:to-cyan-500 shadow-md transition font-medium text-sm flex items-center space-x-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                        ...
                        <div class="text-3xl font-bold text-gray-900">{{ number_format($stats['all']) }}</div>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                        <div class="text-sm text-gray-500 mb-1">New Today</div>
                        <div class="text-3xl font-bold text-blue-600">{{ number_format($stats['new_today'] ?? 0) }}</div>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                        <div class="text-sm text-gray-500 mb-1">Shortlisted</div>
                        <div class="text-3xl font-bold text-green-600">{{ number_format($stats['shortlisted'] ?? 0) }}</div>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                        <div class="text-sm text-gray-500 mb-1">In Interview</div>
                        <div class="text-3xl font-bold text-purple-600">{{ number_format($stats['reviewing'] ?? 0) }}</div>
                    </div>
                </div>

                <!-- Status Navigation Tabs -->
                <div id="statusTabs" class="flex items-center space-x-2 mb-6 overflow-x-auto pb-2">
                    <button onclick="filterByStatus('all')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'all' ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                        </svg>
                        <span>All Applications</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'all' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['all'] }}</span>
                    </button>
                    <button onclick="filterByStatus('rejected')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'rejected' ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="rejected">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                        <span>Rejected</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'rejected' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['rejected'] ?? 0 }}</span>
                    </button>
                    <button onclick="filterByStatus('shortlisted')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'shortlisted' ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="shortlisted">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                        </svg>
                        <span>Shortlisted</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'shortlisted' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['shortlisted'] ?? 0 }}</span>
                    </button>
                    <button onclick="filterByStatus('interview_requested')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'interview_requested' ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="interview_requested">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                        <span>Interview Requested</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'interview_requested' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['interview_requested'] ?? 0 }}</span>
                    </button>
                    <button onclick="filterByStatus('reviewing')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'reviewing' ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="reviewing">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                        <span>Interview</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'reviewing' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['reviewing'] ?? 0 }}</span>
                    </button>
                    <button onclick="filterByStatus('talent_pool')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'talent_pool' ? 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-md' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="talent_pool">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
                        </svg>
                        <span>Talent Pool</span>
                        <span id="talentPoolCount" class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'talent_pool' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['talent_pool'] ?? 0 }}</span>
                    </button>
                </div>

                <!-- Search, Filter & Applications List (one card) -->
                <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div class="relative">
                                ...
                                >
                                    ...
                                    @foreach($jobs as $job)
                                        <option value="{{ $job->id }}" {{ $currentJobId == $job->id ? 'selected' : '' }}>
                                            {{ $job->title }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <!-- Bulk Action Bar -->
                    <div id="bulkActionBar" class="hidden px-5 py-2.5 bg-blue-50 border-b border-blue-100 flex items-center gap-4">
                        <span id="selectedCount" class="text-sm font-medium text-blue-700">0 selected</span>
                        <button onclick="exportSelected()" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-blue-500 to-cyan-400 text-white rounded-md hover:from-blue-600 hover:to-cyan-500 shadow-md transition text-xs font-semibold">
                        ...
                        <button onclick="clearSelection()" class="text-xs text-blue-600 hover:text-blue-800 font-medium">Clear</button>
                    </div>

                    <!-- Applications List -->
                    <div id="applicationsList" class="divide-y divide-gray-100">
                    @forelse($applications as $application)
                        ...
                        @endphp
                        <div class="px-5 py-3.5 hover:bg-gray-50 transition cursor-pointer" onclick="openApplicationModal({{ $application->id }})">
                            <div class="flex items-center gap-4">
                                <!-- Checkbox -->
                                <input type="checkbox" value="{{ $application->id }}" onclick="event.stopPropagation(); toggleSelect(this)" class="app-checkbox w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500 cursor-pointer flex-shrink-0">
                                <!-- Avatar -->
                                @php
                                    ...
                                    $avatarColors = ['from-blue-500 to-blue-600', 'from-purple-500 to-purple-600', 'from-rose-500 to-rose-600', 'from-amber-500 to-amber-600', 'from-teal-500 to-teal-600', 'from-indigo-500 to-indigo-600'];
                                    $colorIndex = crc32($application->first_name . $application->last_name) % count($avatarColors);
                                    $avatarGradient = $avatarColors[abs($colorIndex)];
                                @endphp
                                @if($photoUrl)
                                    <img src="{{ $photoUrl }}" alt="{{ $application->first_name }} {{ $application->last_name }}" 
                                         class="w-11 h-11 rounded-xl object-cover flex-shrink-0"
                                         onerror="this.onerror=null; this.outerHTML='<div class=\'w-11 h-11 rounded-xl bg-gradient-to-br {{ $avatarGradient }} flex items-center justify-center text-white font-bold text-sm flex-shrink-0\'>{{ $initials }}</div>';">
                                @else
                                    <div class="w-11 h-11 rounded-xl bg-gradient-to-br {{ $avatarGradient }} flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
                                        {{ $initials }}
                                    </div>
                                @endif
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2.5 mb-0.5">
                                        <h3 class="text-sm font-bold text-gray-900">{{ $application->first_name }} {{ $application->last_name }}</h3>
                                        <span class="px-2 py-0.5 text-[11px] font-semibold rounded-full {{ $statusColor }} whitespace-nowrap">{{ $statusLabel }}</span>
                                    </div>
                                    <div class="flex items-center gap-1.5 text-xs text-gray-500">
                                        <span class="font-medium text-gray-700">{{ $application->jobAdvertisement->title }}</span>
                                        <span class="text-gray-300">|</span>
                                        <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                                        <span>JOB-{{ str_pad($application->jobAdvertisement->id ?? 0, 3, '0', STR_PAD_LEFT) }}</span>
                                        <span class="text-gray-300">|</span>
                                        <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z"/></svg>
                                        <span>{{ $application->jobAdvertisement->is_remote ? 'Remote' : ($application->jobAdvertisement->location ?? 'Not specified') }}</span>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-5 flex-shrink-0">
                                    ...
                                    <div class="flex items-center gap-1.5 ml-1">
                                        <button onclick="event.stopPropagation(); toggleTalentPool({{ $application->id }}, this)" class="p-1.5 rounded-lg transition {{ $application->in_talent_pool ? 'text-blue-600 bg-blue-50' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50' }}" title="{{ $application->in_talent_pool ? 'Remove from Talent Pool' : 'Add to Talent Pool' }}">
                                            <svg class="w-[18px] h-[18px]" fill="{{ $application->in_talent_pool ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"/></svg>
                                        </button>
...
<div id="applicationModal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.4); backdrop-filter: blur(2px);">
    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div class="px-6 py-4 flex justify-between items-center flex-shrink-0 border-b border-gray-200" style="background-color: #F7F8F9;">
            <h3 class="text-xl font-semibold text-gray-900">Applicant Profile</h3>
            <div class="flex items-center space-x-3">
                <button type="button" id="applicationModalDownloadBtn" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-lg transition" title="Download Resume" style="display: none;">
...
<script>
            ...
            <div class="space-y-6 text-gray-800">
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
                    ...
                    <div class="flex items-start gap-5">
                        ...
                        <div class="flex-shrink-0">
                            ${profilePhoto ? `
                                <img src="${profilePhoto}" alt="${application.first_name} ${application.last_name}" class="w-[88px] h-[88px] rounded-xl object-cover border border-gray-200" onerror="this.onerror=null; this.outerHTML='<div style=\\'width:88px;height:88px\\' class=\\'rounded-xl flex items-center justify-center text-white font-bold text-3xl flex-shrink-0\\' style=\\'background:linear-gradient(135deg,#2563eb 0%,#3b82f6 50%,#60a5fa 100%)\\'>${initials}</div>';">
                            ` : `
                        ...
                        <div class="flex-1 min-w-0">
                            <h4 class="text-[22px] font-bold text-gray-900 leading-tight">${application.first_name} ${application.last_name}</h4>
                            <p class="text-sm text-gray-500 mt-1">${subtitle || 'Applicant'}</p>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div class="bg-gray-100 rounded-xl p-4">
                        <div class="flex items-center gap-2 text-gray-500 mb-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span class="text-xs font-medium">Salary Expectation</span></div>
                        <p class="text-sm font-bold text-gray-900">${jobSeeker.expected_salary || 'Not specified'}</p>
                    </div>
                    <div class="bg-gray-100 rounded-xl p-4">
                        <div class="flex items-center gap-2 text-gray-500 mb-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span class="text-xs font-medium">Availability</span></div>
                        <p class="text-sm font-bold text-gray-900">${jobSeeker.availability || jobSeeker.employment_status ? (jobSeeker.employment_status || '').replace('_', ' ') : 'Not specified'}</p>
                    </div>
                    <div class="bg-gray-100 rounded-xl p-4">
                        <div class="flex items-center gap-2 text-gray-500 mb-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span class="text-xs font-medium">Work Authorization</span></div>
                        <p class="text-sm font-bold text-gray-900">${jobSeeker.work_authorization || 'Not specified'}</p>
                    </div>
                </div>
                ...
                ${jobSeeker.bio ? `
                    <div class="border-b border-gray-200 pb-6">
                        ...
                        <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">${jobSeeker.bio}</p>
                    </div>
                ` : ''}
                ...
                ${jobSeeker && jobSeeker.experiences && Array.isArray(jobSeeker.experiences) && jobSeeker.experiences.length > 0 ? `
                    <div class="border-b border-gray-200 pb-6">
                        <h5 class="flex items-center gap-2 text-lg font-bold text-gray-900 mb-4">
                        ...
                        <div class="space-y-4">
                            ${jobSeeker.experiences.map((exp, i) => {
                                ...
                                return `
                                    <div class="border border-gray-200 border-l-[3px] border-l-blue-500 bg-gray-50 rounded-lg pl-5 pr-4 py-4">
                                        <div class="flex items-start justify-between gap-2">
                                            <p class="text-base font-bold text-gray-900">${exp.job_title || 'Position'}</p>
                                            ${isCurrent ? '<span class="px-3 py-1 text-xs font-semibold rounded-md bg-green-500 text-white whitespace-nowrap">Current</span>' : ''}
                                        </div>
                                        <p class="text-sm text-gray-700 mt-1">${exp.company_name || ''}</p>
                                        ...
                                        ${exp.description ? `<p class="text-sm text-gray-600 mt-3 leading-relaxed">${exp.description}</p>` : ''}
                                    </div>
                                `;
                            }).join('')}
                        </div>
                    ...
                    <div class="border-b border-gray-200 pb-6">
                        ...
                        <div class="space-y-4">
                            ${jobSeeker.educations.map(edu => {
                                ...
                                return `
                                    <div class="border border-gray-200 border-l-[3px] border-l-green-500 bg-gray-50 rounded-lg pl-5 pr-4 py-4 flex items-start justify-between gap-4">
                                        <div class="min-w-0 flex-1">
                                            <p class="text-base font-bold text-gray-900">${edu.degree || 'Degree'}</p>
                                            <p class="text-sm text-gray-700 mt-0.5">${edu.institution_name || ''}</p>
                                            <div class="flex items-center gap-1.5 mt-1.5 text-xs text-gray-500">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg>
                                                ${startDate}${endDate ? ' - ' + endDate : ''}
                                            </div>
                                            ${edu.field_of_study ? `<p class="text-sm text-gray-600 mt-2 leading-relaxed">${edu.field_of_study}</p>` : ''}
                                            ${edu.description ? `<p class="text-sm text-gray-600 mt-1 leading-relaxed">${edu.description}</p>` : ''}
                                        </div>
                                        ${edu.gpa ? `<div class="flex-shrink-0 text-right"><span class="text-xs text-gray-500 block">GPA</span><span class="text-xl font-bold text-gray-900">${edu.gpa}</span></div>` : ''}
                                    </div>
                                `;
                            }).join('')}
                        </div>
                ...
                ${jobSeeker && jobSeeker.skills && Array.isArray(jobSeeker.skills) && jobSeeker.skills.length > 0 ? `
                    <div class="border-b border-gray-200 pb-6">
                        <h5 class="flex items-center gap-2 text-lg font-bold text-gray-900 mb-3">
                            <svg class="w-5 h-5 text-gray-500" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg>
                            Skills
                        </h5>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ${jobSeeker.languages.map(lang => `
                                <div class="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-100">
                                    ...
                                    <div>
                                        <p class="text-sm font-bold text-gray-900">${(lang && (lang.language_name || lang.name)) || 'Language'}</p>
                                        <p class="text-xs text-gray-500">${(lang && lang.proficiency_level) || ''}</p>
                                    </div>
                                </div>
                            `).join('')}
                        </div>
                    ...
                    <div class="border-b border-gray-200 pb-6">
                        ...
                        <div class="space-y-4">
                            ${jobSeeker.certifications.map(cert => {
                                ...
                                return `
                                    <div class="border border-gray-200 border-l-[3px] border-l-green-400 bg-gray-50 rounded-lg pl-5 pr-4 py-4 flex gap-4 items-start">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-sm font-bold text-gray-900">${cert.certification_name || cert.name || 'Certification'}</p>
                                            <p class="text-sm text-gray-600">${cert.issuing_organization || ''}</p>
                                            ${issueDate || expiryDate ? `<p class="text-xs text-gray-500 mt-1">Issued: ${issueDate || 'N/A'}${expiryDate ? '&nbsp;&nbsp;&nbsp;Expires: ' + expiryDate : ''}</p>` : ''}
                    ...
                    <div class="border-b border-gray-200 pb-6">
                        ...
                        <div class="space-y-4">
                            ${jobSeeker.references.map(ref => {
                                ...
                                return '<div class="border border-gray-200 border-l-[3px] border-l-blue-400 bg-gray-50 rounded-lg pl-5 pr-4 py-4 flex gap-4 items-start">' +
                ...
                ${(function() {
                    ...
                    return '<div class="border-b border-gray-200 pb-6">' +
                        ...
                        '<div class="grid grid-cols-2 gap-3">' +
                            docs.map(doc => {
                                return '<div class="flex items-center gap-3 border border-gray-200 rounded-lg px-4 py-3 bg-gray-50">' +
                ...
                ${application.cover_letter ? `
                    <div class="border-b border-gray-200 pb-6">
                        <h5 class="text-lg font-bold text-gray-900 mb-2">Cover Letter</h5>
                        <p class="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">${application.cover_letter}</p>
                    </div>
                ` : ''}

                <!-- Update Status -->
                <div id="applicationModalStatusSection" class="border-b border-gray-200 pb-6">
                    <h5 class="text-sm font-semibold text-gray-700 mb-3">Update Status</h5>
                    <div id="applicationModalStatusRadios" class="flex flex-wrap gap-2">
                        ${['pending','reviewing','shortlisted','interview_requested','hired','rejected'].map(s => `
                            <label class="inline-flex items-center gap-2 px-3 py-2 border rounded-lg cursor-pointer text-sm font-medium transition ${application.status === s ? 'bg-blue-50 border-blue-500 text-blue-700' : 'border-gray-200 text-gray-700 hover:bg-gray-50'}">
                ...
                <div class="border-b border-gray-200 pb-6">
                    ...
                    <div id="requestInterviewFormContainer" style="display:none" class="mt-4 p-4 border border-gray-200 rounded-xl bg-gray-50" data-application-id="">
                        <h6 class="text-sm font-semibold text-gray-800 mb-3">Schedule interview</h6>
                        <form id="requestInterviewForm" onsubmit="submitRequestInterview(event)" class="space-y-3">
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-medium text-gray-600 mb-1">Date</label>
                                    <input type="date" name="scheduled_date" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-600 mb-1">Time</label>
                                    <input type="time" name="scheduled_time" required class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Location (or video link)</label>
                                <input type="text" name="location" placeholder="e.g. Office, or Zoom link" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                            </div>
                            <div>
                                <label class="block text-xs font-medium text-gray-600 mb-1">Notes</label>
                                <textarea name="notes" rows="2" placeholder="Optional instructions for the candidate" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"></textarea>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                <button type="button" onclick="hideRequestInterviewForm()" class="px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium">Cancel</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    async function updateApplicationStatus(applicationId, status) {
        ...
        if (typeof window.showConfirmDialog === 'function') {
            ...
            confirmed = window.confirm('Change status to "' + (label.charAt(0).toUpperCase() + label.slice(1)) + '"?');
        }
        if (!confirmed) return;
    ...
    function loadApplications() {
        ...
        // Show loading state
        applicationsList.className = 'divide-y divide-gray-100';
                ...
                <div class="px-5 py-3.5 hover:bg-gray-50 transition cursor-pointer" onclick="openApplicationModal(${application.id})">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-1.5 text-xs text-gray-500">
                                <span class="font-medium text-gray-700">${application.job_title}</span>
                                <span class="text-gray-300">|</span>
                                <svg class="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/></svg>
                                <span>${jobId}</span>
                                <span class="text-gray-300">|</span>
