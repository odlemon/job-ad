        ...
        <main class="flex-1 p-8 ml-64">
            <div class="max-w-7xl mx-auto">
                ...
                <div id="statusTabs" class="flex items-center space-x-2 mb-6 overflow-x-auto pb-2">
                    <button onclick="filterByStatus('all')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'all' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="all">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="filterByStatus('rejected')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'rejected' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="rejected">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="filterByStatus('shortlisted')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'shortlisted' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="shortlisted">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="filterByStatus('reviewing')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'reviewing' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="reviewing">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="filterByStatus('hired')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'hired' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="hired">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                        </svg>
                        <span>Selected</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'hired' ? 'bg-blue-500' : 'bg-gray-100' }}">{{ $stats['hired'] ?? 0 }}</span>
                    </button>
                    <button onclick="filterByStatus('talent_pool')" class="status-tab-btn px-4 py-2.5 rounded-lg font-medium text-sm flex items-center space-x-2 {{ $currentStatus === 'talent_pool' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 border border-gray-200 hover:bg-gray-50' }} transition whitespace-nowrap" data-status="talent_pool">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"></path>
                        </svg>
                        <span>Talent Pool</span>
                        <span class="px-2 py-0.5 text-xs rounded-full {{ $currentStatus === 'talent_pool' ? 'bg-blue-500' : 'bg-gray-100' }}">0</span>
                    </button>
                </div>
                                ...
                                <div class="flex items-center space-x-4 flex-1 min-w-0">
                                    ...
                                    @if($photoUrl)
                                        <img src="{{ $photoUrl }}" alt="{{ $application->first_name }} {{ $application->last_name }}" 
                                             ...
                                             onerror="this.onerror=null; this.outerHTML='<div class=\'w-12 h-12 rounded-md bg-gradient-to-b from-blue-600 to-blue-400 flex items-center justify-center text-white font-bold text-sm flex-shrink-0\'>{{ $initials }}</div>';">
                                    @else
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center space-x-2 text-sm text-gray-900 mb-2 flex-wrap">
                                            <span class="font-medium">{{ $application->jobAdvertisement->title }}</span>
                                            <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                                            </svg>
                                            <span class="text-gray-500">JOB-{{ str_pad($application->jobAdvertisement->id, 3, '0', STR_PAD_LEFT) }}</span>
                                            <svg class="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                            </svg>
                                            <span class="text-gray-600">{{ $application->jobAdvertisement->is_remote ? 'Remote' : ($application->jobAdvertisement->location ?? 'Not specified') }}</span>
                                        </div>
