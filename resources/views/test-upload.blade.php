        ...
        <div class="mb-8">
            ...
            <form id="singleUploadForm" class="space-y-4">
                <div>
                    ...
                    <input type="file" name="file" id="singleFile" class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Upload Type (optional)</label>
                    <select name="uploadType" id="singleUploadType" class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg">
                ...
                <div id="singleResult" class="mt-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg hidden"></div>
            </form>
        </div>
        ...
        <div>
            ...
            <form id="multipleUploadForm" class="space-y-4">
                <div>
                    ...
                    <input type="file" name="files[]" id="multipleFiles" multiple class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Upload Type (optional)</label>
                    <select name="uploadType" id="multipleUploadType" class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg">
                        <option value="temp">Temp</option>
                        <option value="cv">CV</option>
                        <option value="profile-photos">Profile Photos</option>
                        <option value="company-logos">Company Logos</option>
                ...
                <div id="multipleResult" class="mt-4 p-4 bg-gray-50 dark:bg-gray-900 rounded-lg hidden"></div>
            </form>
        </div>
    </div>
                    ...
                    data.data.files.forEach((file, index) => {
                        filesHtml += `
                            <div class="border border-gray-200 dark:border-gray-700 rounded p-2">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">URL: <a href="${file.url}" target="_blank" class="text-blue-600 hover:underline">${file.url}</a></p>
                                <p class="text-sm text-gray-600 dark:text-gray-400">Size: ${(file.size / 1024).toFixed(2)} KB</p>
                            </div>
                        `;
                    });
                    filesHtml += '</div>';
