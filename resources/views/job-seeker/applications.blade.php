            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="space-y-4">
                    @forelse($applications as $application)
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition application-card" data-status="{{ $application->status }}" data-filter="{{ $status['filter'] }}">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-shrink-0">
                                    @if($company && $company->logo)
                                        ...
                                        @endphp
                                        <img src="{{ $logoUrl }}" alt="{{ $company->name }}" class="w-14 h-14 rounded-lg object-cover" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\'w-14 h-14 rounded-lg bg-gray-100 flex items-center justify-center\'><span class=\'text-lg font-semibold text-gray-600\'>{{ substr($company->name ?? 'C', 0, 1) }}</span></div>';">
                                    @else
                                        <div class="w-14 h-14 rounded-lg bg-gray-100 flex items-center justify-center">
                                            <span class="text-lg font-semibold text-gray-600">{{ substr($company->name ?? 'C', 0, 1) }}</span>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-start justify-between mb-3">
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-sm text-gray-600">{{ $company->name ?? 'Unknown Company' }}</p>
                                        </div>
                                        <!-- Status Badge -->
                                        <span class="ml-4 px-3 py-1 rounded-md text-xs font-medium whitespace-nowrap {{ $status['color'] }}">
                                    ...
                                    @if($application->status === 'shortlisted' && $application->interview_date)
                                        <div class="bg-purple-50 border border-purple-200 rounded-md px-3 py-2.5 mb-3">
                                            <div class="flex items-center gap-2">
                                                <svg class="w-4 h-4 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                                </svg>
                                                <span class="text-sm text-purple-900">Interview Scheduled: {{ \Carbon\Carbon::parse($application->interview_date)->format('M j, Y \a\t g:i A') }}</span>
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    <div class="flex items-center gap-3">
                                        <button type="button" onclick="openApplicationModal({{ $application->id }})" 
                                            class="px-5 py-2 bg-blue-600 text-white rounded-md text-sm font-medium hover:bg-blue-700 transition">
                                            View Details
                                        </button>
                                        <button type="button" onclick="openAddNoteModal({{ $application->id }})" 
